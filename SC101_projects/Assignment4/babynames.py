...
This file reads files and adds texts to dictionary
"""
    ...
    if name in name_data:
        if year in name_data[name]:
            if int(rank) < int(name_data[name][year]):
                name_data[name][year] = rank                     # update lower number rank
        else:
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        year = f.readline().strip()                             # first line
        for line in f:
            info_lst = line.split(",")
...
def search_names(name_data, target):
    ...
    for key in name_data:
        if target.lower() in key.lower():               # case-insensitive
            matching_names += [key]
